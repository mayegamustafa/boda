...
class OrderDetailsView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return ref
        .watch(orderDetailsProvider(orderID))
        .when(
          data: (data) {
            return Scaffold(
              appBar: AppBar(
...
class BottomSection extends ConsumerStatefulWidget {
  ...
  BottomSection({required this.orderDetailsModel});
  @override
...
class _BottomSectionState extends ConsumerState<BottomSection> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      } else if (widget.orderDetailsModel.data!.order!.orderStatus ==
          'Delivered') {
        ref.read(slideButtonLeftPosition.notifier).state = 328.w - 40.r;
        ref.read(slideButtonComplete.notifier).state = true;
      }
    });
    // if (widget.orderDetailsModel.data!.order!.orderStatus == 'Delivered') {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: context.isDark ? Colors.grey.withOpacity(0.1) : Colors.white,
      ),
      child: Container(
        width: 328.w,
        height: 54.h,
      ...
      child: Column(
        ...
        children: [
          Text(
            ...
            style: AppTextStyle.normalBody.copyWith(
              color: context.isDark ? Colors.white : Color(0xFF05161B),
              fontSize: 14.sp,
              fontWeight: FontWeight.w700,
            ),
          ),
                        ...
                        children: [
                          Container(
                            margin: EdgeInsets.only(
                              left: shopMapVisibility(status) ? 10.r : 0.r,
                            ),
                            padding: EdgeInsets.all(10.r),
                            decoration:
                                shopMapVisibility(status)
                                    ? ShapeDecoration(
                                      color:
                                          context.isDark
                                                ...
                                                child: RichText(
                                                  text: TextSpan(
                                                    children: [
                                                      ...
                                                      TextSpan(
                                                        text:
                                                            ' (${orderDetailsModel.data?.order?.shop?.phone})',
                                                        style: TextStyle(
                                                          color: Color(
                                                            0xFF617885,
                                                          ),
                  ...
                  Expanded(
                    child: Container(
                      ...
                      padding: EdgeInsets.all(
                        userMapVisibility(status) ? 10.r : 0.h,
                      ),
                      decoration:
                          userMapVisibility(status)
                              ? ShapeDecoration(
                                color:
                                    context.isDark
                                        ? Colors.grey.withOpacity(0.1)
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            "${orderDetailsModel.data?.order?.user?.address?.flatNo ?? ''}, ${orderDetailsModel.data?.order?.user?.address?.area ?? ''},${orderDetailsModel.data?.order?.user?.address?.addressLine ?? ''}, ${orderDetailsModel.data?.order?.user?.address?.addressLine2 ?? ''}, ${orderDetailsModel.data?.order?.user?.address?.postCode ?? ''}",
                            style: TextStyle(
                              color: Color(0xFF617885),
                              fontSize: 12.sp,
                              fontWeight: FontWeight.w400,
...
class _OrderInfor extends StatelessWidget {
  const _OrderInfor({required this.orderDetailsModel});
          ...
          Row(
            ...
            children: [
              ...
              Text(
                ...
                textAlign: TextAlign.center,
                style: AppTextStyle.largeBody.copyWith(
                  color: context.isDark ? Color(0xFF617885) : Color(0xFF05161B),
                  fontSize: 24.sp,
                  fontWeight: FontWeight.w700,
                ),
              ),
...
class PaymentStatusCard extends StatelessWidget {
  ...
  const PaymentStatusCard({Key? key, required this.orderStatus})
    : super(key: key);
